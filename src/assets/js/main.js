...
const getTarget = (forceTarget) => {
	const targetThingIndex =
		forceTarget || Math.floor(Math.random() * currentThings.length);
...
const stop = (forceTarget) => {
	...
	if (!el.classList.contains('stopping')) {
		...
		const spinToStop = [
			...
			{
				transform: `perspective(none) rotate(${
					targetRotation + 360 * extraSpins
				}deg)`,
			},
		];
